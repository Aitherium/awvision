...
Rules:
  * **MOAT001** no monorepo import (`lib.`, `services.`, `from AitherOS`). awvision
...
archive) — never 0 for "I could not look".
    ...
    python scripts/check_moat_boundary.py [dist/awvision-*.whl dist/*.tar.gz ...]
...
def self_test() -> int:
    ...
    def wheel(members: dict) -> Path:
        p = Path(tempfile.mkdtemp()) / "awvision-0.0.0-py3-none-any.whl"
    ...
    check("MOAT001 catches a monorepo import",
          any(f.startswith("MOAT001") for f in
              inspect(wheel({**clean, "awvision/x.py": "from lib.core import X\n"}))), True)

    check("MOAT002 catches a debt id",
          any("debt-ledger row id" in f for f in
              inspect(wheel({**clean, "awvision/x.py": "# see D-0000\n"}))), True)

    check("MOAT002 catches a checker rule id",
          any("checker rule id" in f for f in
              inspect(wheel({**clean, "awvision/x.py": "# MOAT001 says so\n"}))), True)
    ...
    check("MOAT003 refuses an artifact missing a keystone",
          any(f.startswith("MOAT003") for f in
              inspect(wheel({"awvision/cli.py": "x=1\n"}))), True)

    # The package's own imports should not be flagged.
    check("does NOT flag the package's own imports",
          inspect(wheel({**clean, "awvision/y.py": "from awvision.client import call\n"})) == [],
          True)
    ...
    try:
        broken = Path(tempfile.mkdtemp()) / "awvision-0.0.0.tar.gz"

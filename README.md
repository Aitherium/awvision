...
**[Docs](https://aitherium.github.io/awvision/)**  ·  [Source](https://github.com/Aitherium/awvision)  ·  `pip install awvision`  ·  [The Aither World](https://aitherium.github.io/)

> **The Aither World** is an operating system for agents — a Linux you can hand to one, the runtimes it works in, and the tools it works with. [awnix](https://github.com/Aitherium/awnix) is the Linux underneath it; **awvision** is one of its 55 bricks — each installs on its own, runs offline, and needs no account.
>
...
```bash
export AWVISION_URL=http://100.64.0.38:8124
export AWVISION_MODEL=gemma4-12b
```

The defaults point at the AitherOS fleet's vision lane — gemma4-12b on the
DGX Spark (`claude_bridge.yaml`). Point `AWVISION_URL` at any other
OpenAI-compatible vision endpoint to override.

Or pass them as options:

```bash
awvision --endpoint http://100.64.0.38:8124 --model gemma4-12b ask image.png "What's this?"
```
...
| **awvision** _(you are here)_ | a filename and a caption somebody wrote | what a model actually reports about the pixels |
...
| **awvision** _(you are here)_ | See an image — describe it, ask it a question, compare two | [docs](https://aitherium.github.io/awvision/) |
...
<div id="aither-constellation" data-self="awvision"></div>
